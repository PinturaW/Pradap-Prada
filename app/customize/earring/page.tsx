...
function EarringCustomizerContent() {
  ...
  const fixedDesign = fixedDesignByType.earring;
  const productDetail = fixedProductDetailByType.earring;
  ...
  const buyNowHref = selectedGemType
    ? `/checkout?type=earring&gemType=${selectedGemType}`
    : "/checkout?type=earring";
        ...
        <div className="grid gap-8 lg:grid-cols-[1.3fr_0.9fr]">
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ maxHeight: flowerOpen ? 300 : 0, overflow: "hidden", transition: "max-height 300ms ease" }}>
                  <p style={{ fontSize: 13, color: "#666666", lineHeight: 1.9, fontStyle: "italic", paddingTop: 12, maxWidth: 420 }}>
                    The Lamduan blooms quietly at dusk, releasing its sweetness only when the world slows down. A gentle flower for women of grace, noticed not by noise, but by presence.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
              ...
              <div className="mt-5 border border-[#EBEBEB] p-4">
                ...
                <div className="mt-2 flex items-center justify-between text-[13px] font-light">
                  ...
                  <span className="text-[#2A2A2A]">
                    ฿{((gemBasePrices[selectedGemType] ?? 0) + categoryAdjustments.earring).toLocaleString()}
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {selectedGemType ? (
                <Link
                  href={`/cart?type=earring&gemType=${selectedGemType}`}
...
export default function EarringCustomizerPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-white" />}>
      <EarringCustomizerContent />
    </Suspense>
  );
}
