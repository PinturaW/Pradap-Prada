...
function NecklaceCustomizerContent() {
  ...
  const fixedDesign = fixedDesignByType.necklace;
  const productDetail = fixedProductDetailByType.necklace;
  ...
  const buyNowHref = selectedGemType
    ? `/checkout?type=necklace&gemType=${selectedGemType}`
    : "/checkout?type=necklace";
        ...
        <div className="grid gap-8 lg:grid-cols-[1.3fr_0.9fr]">
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ maxHeight: flowerOpen ? 300 : 0, overflow: "hidden", transition: "max-height 300ms ease" }}>
                  <p style={{ fontSize: 13, color: "#666666", lineHeight: 1.9, fontStyle: "italic", paddingTop: 12, maxWidth: 420 }}>
                    The Ylang Ylang — flower of flowers — has perfumed the world&apos;s most iconic fragrances for centuries. Long petals that curl softly, a scent that lingers long after you have left the room.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
              ...
              <div className="mt-5 border border-[#EBEBEB] p-4">
                ...
                <div className="mt-2 flex items-center justify-between text-[13px] font-light">
                  ...
                  <span className="text-[#2A2A2A]">
                    ฿{((gemBasePrices[selectedGemType] ?? 0) + categoryAdjustments.necklace).toLocaleString()}
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {selectedGemType ? (
                <Link
                  href={`/cart?type=necklace&gemType=${selectedGemType}`}
...
export default function NecklaceCustomizerPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-white" />}>
      <NecklaceCustomizerContent />
    </Suspense>
  );
}
