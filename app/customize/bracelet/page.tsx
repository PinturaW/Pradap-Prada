...
import { ChevronDown, Gem, Handshake, Package, ShieldCheck } from "lucide-react";
import {
  fixedDesignByType,
...
const braceletSizeOptions = [
  { value: "150", label: "150mm (5.1 - 5.5 in)" },
  { value: "160", label: "160mm (5.5 - 5.9 in)" },
  { value: "170", label: "170mm (5.9 - 6.3 in)" },
  { value: "180", label: "180mm (6.3 - 6.7 in)" },
  { value: "190", label: "190mm (6.7 - 7.1 in)" },
];

function getBraceletSizeLabel(size: string | null) {
  if (!size) return null;
  return braceletSizeOptions.find((option) => option.value === size)?.label ?? `${size}mm`;
}

function BraceletCustomizerContent() {
  ...
  const [selectedBraceletSize, setSelectedBraceletSize] = useState<string | null>(null);
  const [flowerOpen, setFlowerOpen] = useState(false);
  const fixedDesign = fixedDesignByType.bracelet;
  const productDetail = fixedProductDetailByType.bracelet;
  ...
  const buyNowHref = selectedGemType && selectedBraceletSize
    ? `/checkout?type=bracelet&gemType=${selectedGemType}&braceletSize=${selectedBraceletSize}`
    : "/checkout?type=bracelet";
        ...
        <div className="grid gap-8 lg:grid-cols-[1.3fr_0.9fr]">
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ maxHeight: flowerOpen ? 300 : 0, overflow: "hidden", transition: "max-height 300ms ease" }}>
                  <p style={{ fontSize: 13, color: "#666666", lineHeight: 1.9, fontStyle: "italic", paddingTop: 12, maxWidth: 420 }}>
                    The Kritsana is Trat&apos;s provincial flower — rare, dark, and quietly magnetic. One of Thailand&apos;s most precious blooms, known for its deep intoxicating fragrance.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="mt-4 border border-[#EBEBEB] p-4">
              <p className="text-[12px] font-light tracking-[0.14em] uppercase text-[#9A9A9A]">Bracelet Size</p>
              ...
              <div className="relative mt-4">
                <select
                  ...
                  onChange={(event) => setSelectedBraceletSize(event.target.value || null)}
                  className="h-11 w-full appearance-none border border-[#EBEBEB] bg-white pl-3 pr-10 text-[12px] font-light text-[#2A2A2A] outline-none transition-colors focus:border-[#C4956A]"
                >
                  <option value="" className="text-[#9A9A9A]">Select bracelet size</option>
                  {braceletSizeOptions.map((option) => (
              ...
              <div className="mt-5 border border-[#EBEBEB] p-4">
                ...
                {selectedBraceletSize && (
                  <div className="mt-2 flex items-center justify-between text-[13px] font-light">
                    <span className="text-[#9A9A9A]">Bracelet size</span>
                    <span className="text-right text-[#2A2A2A]">{getBraceletSizeLabel(selectedBraceletSize)}</span>
                  </div>
                )}
                <div className="mt-2 flex items-center justify-between text-[13px] font-light">
                  <span className="text-[#9A9A9A]">Estimated total</span>
                  <span className="text-[#2A2A2A]">
                    ฿{((gemBasePrices[selectedGemType] ?? 0) + categoryAdjustments.bracelet).toLocaleString()}
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {selectedGemType && selectedBraceletSize ? (
                <Link
                  href={`/cart?type=bracelet&gemType=${selectedGemType}&braceletSize=${selectedBraceletSize}`}
